...
- [x] **Procedural multi-biome world** (`src/world/{rng,palette,colliders,heightmap,map}.ts` + `src/world/biomes/*`): 400×400m heightmap-based world with 5 biomes — Ruined City (NW), Industrial Complex (NE), Dried Dam (SW), Burnt Forest (SE), Mountain Ridge (E). Heightmap 200×200 vertex grid, vertex-colored terrain, exposed `groundHeight(x,z)` sampler. Hero landmarks: fallen radio tower, broken cathedral spire, leaning 45m smokestack, 25m dam wall, observatory with red beacon. Walkable interiors in two city hero buildings, industrial hangar with catwalk, pump house, ranger cabin. 4 shelters (one per biome). Distant city + mountain silhouettes for parallax. ~25 PointLights, ~1500+ instanced decorations.
...
- [x] **FPS controller** (`src/systems/fps-controller.ts`): pointer lock, WASD, sprint, crouch toggle, jump, gravity, world bounds, **AABB collision via `pushOutXZ`** (radius 0.45m, height-aware vertical overlap test), **heightmap ground sampling via `getGroundHeight` option**. `setColliders(c)` to swap at runtime.
...
- [x] **main.ts** wires: scene + atmosphere + world (multi-biome map) + FPS + colliders + debug. Player spawns at random shelter (city/industrial/dam/forest). World update closure runs each frame for fire flicker + interior light flicker + observatory pulse.
