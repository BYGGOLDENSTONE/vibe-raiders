...
### Wave 12 — deployment + jam submission (this session)

The game is live for Cursor Vibe Jam 2026 at **https://vibecoders-guide-to-the-galaxy.byggoldenstone.partykit.dev**.

- **Combined hosting** — PartyKit serves both the websocket relay AND the built SPA from the same domain. `partykit.json` adds `"serve": "dist"`, so `npm run party:deploy` uploads `dist/` (HTML + JS + CSS + favicon + `music/conquerer.mp3`) alongside the relay worker. One URL → game + multiplayer; no separate static host.
- **Production env** — `.env.production` sets `VITE_PARTYKIT_HOST=vibecoders-guide-to-the-galaxy.byggoldenstone.partykit.dev`. `setupMultiplayer` in `src/galaxy/app.ts` already read this var with a `localhost:1999` fallback, so `npm run dev` + `npm run party:dev` keeps working unchanged.
- **Project rename** — the partykit project name was `vibe-raiders` (an older internal codename); deploy was redone under `vibecoders-guide-to-the-galaxy` so the public URL matches the game title. The old `vibe-raiders.byggoldenstone.partykit.dev` subdomain stays alive until manually deleted (`npx partykit delete vibe-raiders`).
- **Build output** — 692 KB JS + 39 KB CSS + 0.6 KB HTML + 1.89 MB MP3 ≈ 2.6 MB total transfer. Vite's "chunk > 500 KB" warning is mostly Three.js; code-splitting deferred since instant-load is already <3 s on a fresh visit.

**Files touched:** `partykit.json` (`serve: "dist"` + project rename), `.env.production` (new — production relay host), `CLAUDE.md` (this entry).

### Wave 11 — fullscreen 2D map overlay (previous session)
...
| **W12** | ✅ Complete. Combined PartyKit deployment — `serve: "dist"` ships the static SPA alongside the websocket relay so the jam URL is a single domain (`vibecoders-guide-to-the-galaxy.byggoldenstone.partykit.dev`). `.env.production` wires `VITE_PARTYKIT_HOST`. Project renamed from the old `vibe-raiders` codename. |
...
- Storage keys to know:
  ...
  - `vibecoder.empire.mp.v5` — multiplayer empire state (separate slot, fresh on first MP launch).
